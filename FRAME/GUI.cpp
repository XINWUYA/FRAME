...
bool IGUI::combo(const std::string& vLabel, int &vioCurrentItem, const std::vector<std::string>& vItems)
{
	...
	return ImGui::Combo(vLabel.c_str(), &vioCurrentItem, Items.data(), vItems.size());
}
...
{
	ImGui::PlotLines(vLabel.c_str(), vValueSet.data(), vValueSet.size(), 0, nullptr, vMin, vMax, ImVec2(vSize[0], vSize[1]));
}
...
{
	ImGui::PlotHistogram(vLable.c_str(), vValueSet.data(), vValueSet.size(), 0, nullptr, vMin, vMax, ImVec2(vSize[0], vSize[1]));
}
...
bool IGUI::listBox(const std::string& vLabel, int &voCurrentItem, const std::vector<std::string>& vItems, int vDisplayedNumInBox/* = -1*/)
{
	...
	for (const auto &e : vItems)
		Items.push_back(e.c_str());
	return ImGui::ListBox(vLabel.c_str(), &voCurrentItem, Items.data(), vItems.size(), vDisplayedNumInBox);
}
