...
vec3 DiffuseAndSpecularFromMetallic (vec3 albedo, float metallic, out vec3 specColor)
{
	...
	//vec3 F0 = vec3(0.04);
	//specColor = mix(F0, albedo, metallic);
...
void main()
{
	...
	mat3 TBNMatrix = mat3(v2f_Tangent, Bitangent, v2f_Normal);
	...
	vec3 GroundNormal = FetchNormal(v2f_TexCoords, TBNMatrix);
	GroundNormal = normalize(CorrectNormal(GroundNormal, ViewDir));
	...
	vec3 DiffuseColor = DiffuseAndSpecularFromMetallic (Albedo, Metallic, /*out*/ SpecularColor);
	vec3 ResultColor;
