...
vec4 getColorByRadius(int vTextureIndex)
{
	...
	/*else
	{
		ResultColor = textureLod(u_InputImage, v2f_TexCoords, vTextureIndex).rgb * u_GaussianWeight[0];
		if (u_IsHorizontal)
		{
			for (int i = 1; i < 5; ++i)
			{
				ResultColor += textureLod(u_InputImage, v2f_TexCoords + vec2(TextureOffset.x * i, 0.0f), vTextureIndex).rgb * u_GaussianWeight[i];
				ResultColor += textureLod(u_InputImage, v2f_TexCoords - vec2(TextureOffset.x * i, 0.0f), vTextureIndex).rgb * u_GaussianWeight[i];
			}
		}
		else
		{
			for (int i = 1; i < 5; ++i)
			{
				ResultColor += textureLod(u_InputImage, v2f_TexCoords + vec2(0.0f, TextureOffset.y * i), vTextureIndex).rgb * u_GaussianWeight[i];
				ResultColor += textureLod(u_InputImage, v2f_TexCoords - vec2(0.0f, TextureOffset.y * i), vTextureIndex).rgb * u_GaussianWeight[i];
			}
		}
	}*/
...
void main()
{
    //vec2 TexelOffset = 1.0 / textureSize(u_InputImage, 0); //每个纹素的大小
    //vec3 Result = textureLod(u_InputImage, v2f_TexCoords, u_MipLevel).rgb * u_GaussianWeight[0];
    //if(u_IsHorizontal)
    //{
    //    for(int i = 1; i < 5; ++i)
    //    {
