...
考虑可能会出现的以下两种情况：1）当点光源$\omega_1$的初始位置于点P所在半球面以下，而在乘上变换矩阵${M_1}^{-1}$ 后，光源位置移动到半球面以上；2）当点光源$\omega_2$ 的初始位置于点P所在半球面以上，而在乘上变换矩阵${M_2}^{-1}$ 后，光源位置移动到半球面以下。
...
按照实际中的真实效果，当点光源位于平面以下时，平面表面应当无任何光照效果。而当出现情况1时，使用两种方法所呈现的效果如图\ref{fig:LightClip}中$(c)$和$(d)$，其中光源初始位置为$(0, 0, -0.158731)$，地面粗糙度为$\alpha=0.5$，可以看出使用LTC 的方法在平面上有明显的“余光”，这是因为虽然$\omega_1.z < 0$，但由式\ref{eq:SpecularOmega}和\ref{eq:DiffuseOmega}，$\omega_s.z > 0$，$\omega_d.z > 0$，代入式\ref{eq:LTCSinglePointLight}，有$L_o > 0$。 因此对于这类光源，我们需要在计算时裁剪去掉。很明显当点光源初始位置在平面以下时，有
\begin{equation}
  \omega_1\cdot n<0
\end{equation}

对于第2种情况，当$\omega_2$乘上变换矩阵${M_2}^{-1}$后，光源位置移动到半球面以下，此时由式\ref{eq:SpecularOmega}和\ref{eq:DiffuseOmega}，有$\omega_s.z < 0$和$\omega_d.z < 0$，代入式\ref{eq:LTCSinglePointLight}，有$L_o < 0$，不符合真实情况，因此此类光源也需要进行裁剪。
综上，整合所有情况之后的式\ref{eq:FinalEquation4SPL}应为：
\begin{equation}
    L_o=
    \begin{cases}
    0, &\omega_i\cdot n<0 \\
    \frac{\max(\omega_s.z, 0)}{(\omega_s\cdot\omega_s)^2}|M_s^{-1}|L_i+\frac{\max(\omega_d.z, 0)}{(\omega_d\cdot\omega_d)^2}|M_d^{-1}|L_i, &\omega_i\cdot n>0
    \end{cases}
\end{equation}
