...
现有的分块着色、分簇着色算法以及Forward+等等渲染框架里，它们的核心思想都是光源剔除，可以将本文提出的多光源光照模型结合到这些算法及框架中，快速实现基于物理的大量点光源光照。也有很多使用大量虚拟点光源来近似间接光照的全局光照算法，比如Forward Light Cuts算法\cite{Laurent2016Forward}，但是这些算法里通常使用的间接光照模型都是lamber漫反射模型，无法准确地表示出间接光的光泽效果，可以考虑使用并改进本文的算法，在光线的一次反射以及二次反射里，使用分布近似以及本文的光照模型来快速实现，这也是我们想要重点研究的方向。
