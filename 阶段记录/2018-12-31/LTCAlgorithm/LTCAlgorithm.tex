...
In scene rendering, it's still difficult to guarantee both the effect and speed when the number of light sources is large, especially when the lights are dynamic. In recent years, there are many algorithms for rendering a large number of point sources in the scene in realtime, such as Forward+\cite{harada2012forward+} algorithm divided the screen into tiles to culling the lights. In order to solve the problem of too many lights in tiles and the depth's discontinuity, clustering shading\cite{Olsson2012Clustered} on the basis of tiled shading, the tiles are extended from two-dimensional screen space to the three-dimensional observation space, further divided in depth.  LLL\cite{bezrati2018real} algorithm creates a GPU light link list for each pixel, which enables culling lights as well as rendering opaque and translucent objects simultaneously in one pass in deferred shading, makes it even simpler for shading the translucent objects and particles.

\subsection{Real-time Lighting Model}
There are many lighting models for point light source rendering. Bouknight has proposed the Lambert model \cite{Bouknight1970A}, which gives the treatment of rough surfaces. The Phong\cite{phong1975illumination} model incorporates specular reflection, and the Blinn-Phong model \cite{blinn1977models} is an extension of the Phong model, which corrects the specular light effect of the Phong model and is more computationally efficient. However, they all do not conform to the laws of physics. In physics-based rendering, Bidirectional Reflection Distribution Function \cite{Nicodemus1965Directional} is usually used to represent the ratio of the reflected radiance to the incident irradiance of the object surface. BRDF functions that follow the laws of physics perfectly are very complicated, therefore approximate methods are usually used in actual rendering. Different approximation methods form different BRDF models. There are already many ideal BRDF models \cite{Montes2012AnOO}, such as Granier-Heidrich BRDF model\cite{Granier2003A} based on layering theory, Oren-Nayar model\cite{Nayar1995Visual} based on geometric viewpoint, Schlick BRDF model\cite{schlick1994inexpensive}based on experiment, etc.. But the most used in real-time rendering and game engine is the Cook-Torrance BRDF model\cite{cook1982reflectance} based on the micro-surface theory proposed by Cook et al. on Beckman\cite{Beckmann1963The} and Torrance-Sparrow\cite{torrance1967theory}, the theory holds that any surface is composed of very small microscopic facets, each with its own roughness.In addition to this, there are many variants of the Cook-Torrance BRDF model \cite{Pont2002Bidirectional}\cite{schlick1994inexpensive}\cite{kelemen2001microfacet}, which have also been experimentally proven to mimic the real surface \cite{Ngan2005Experimental}. For the microscopic surface normal distribution function in this model, some distribution functions such as Beckmann\cite{Beckmann1963The}, Phong\cite{phong1975illumination}, and GGX\cite{Walter2007Microfacet} are very popular, but only the GGX distribution will attenuate faster at the center of the highlight, and the attenuation will become slower when away from the highlight center, making the GGX highlight has a tailing effect, which is in line with the reality\cite{Matusik2003A}. The bidirectional shadowing-masking function can be obtained by the normal distribution function and the Smith Shadowing function, but the Smith shadow function assumes that the shadowing and the masking are not related, but more popular in recent years is the more precise geometric occlusion function proposed by Lagarde et al. \cite{lagarde2014moving}, which considers the correlation between shadowing and masking. For the diffuse reflection BRDF, the Lambert BRDF model \cite{Bouknight1970A} was once popular, but because it does not consider directionality, and cannot effectively reflect the effect of retro-reflection, Disney has proposed a new diffuse reflection empirical model\cite{burley2012physically}.

\subsection{Linearly Transformed Cosine}
Since the physics-based BRDF model is complex and computationally intensive, Eric Heitz et al. proposed a mathematical method called Linearly Transformed Cosine to achieve surface light source illumination, using a linear matrix and cosine distribution to fit BRDF distribution. This work is inspired by this, using Linearly Transformed Cosine to reduce the calculation of the physics-based illumination of a large number of light sources.
